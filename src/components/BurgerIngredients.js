...
function BurgerIngredients(props) {
  ...
  return (
    <div className={styles.burgerIngredients}>
      ...
      <div className={styles.ingredients} style={{ height: 'calc(100% - 156px)'}}>
        <p className={`text text_type_main-medium ${styles.type} mt-10`}>Булки</p>
        {props.data.filter(item => item.type === "bun").map((item, index)=>(
          <BurgerIngredient key={item._id} counter={1} image={item.image} price={item.price} name={item.name} extraClass="ml-4 mr-2 mb-2 mt-6" />
          )
        )}
